...
test.before(async () => {
  try {
    await db.migrate();
    await db.run(
      'TRUNCATE snapshot_orders, snapshot_days, snapshot_totals, snapshot_runs, payouts, sales, imports, login_tokens, audit_log, outbox, creators RESTART IDENTITY CASCADE'
    );
  } catch (err) {
    available = false;
    console.error(
...
async function seedCreator(code, rate) {
  return db.one(
    `INSERT INTO creators (full_name, email, email_norm, instagram, requested_code,
     ...
     VALUES ($1,$2,$3,$4,$5,$5,$5,'approved',$6,10,'test', now()) RETURNING *`,
...
dbTest('Provision wird nur auf bestätigte Bestellungen gerechnet', async () => {
  ...
  const result = await importSalesCsv(csv, { filename: 'test.csv', actor: 'test' });
...
dbTest('das Dashboard ändert sich erst mit dem nächsten Snapshot', async () => {
  ...
  const today = localDate();
  await importSalesCsv(`order_ref;code;datum;total;status\n20;CCC15;${today};100,00;paid`, { actor: 'test' });
  await buildSnapshot({ triggeredBy: 'test' });
  const before = (await dashboardFor(creator.id)).totals.revenue_total;

  await importSalesCsv(`order_ref;code;datum;total;status\n21;CCC15;${today};500,00;paid`, { actor: 'test' });
...
dbTest('bereits ausgezahlte Provision senkt den offenen Betrag', async () => {
  ...
  const today = localDate();
  await importSalesCsv(`order_ref;code;datum;total;status\n40;EEE10;${today};1000,00;paid`, { actor: 'test' });
  await db.run(
...
dbTest('ohne SMTP landet jede Nachricht samt Link im Ausgangspostfach', async () => {
  ...
  const result = await mailer.send({
    to: creator.email,
    ...mailer.templates.approved({ ...creator, assigned_code: 'GGG10' }, link),
  });
